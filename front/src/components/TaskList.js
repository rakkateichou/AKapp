...
const TaskList = (props) => {
    ...
    const handleDeleteTask = (e, task) => {
        ...
        setTasks(tasks.filter(item => item.taskEntity.id !== task.id))
    }
    ...
    return (<>
        ...
        <Paper elevation={3} style={paperStyle}>
            ...
            {tasks.map(task => (
                <TaskPaper key={task.taskEntity.id} response={task} hasRating={props.user.login !== undefined} deletable onDelete={(e) => {handleDeleteTask(e, task.taskEntity)}} handleStarTask={(e) => { if (!task.isFavorite) props.handleStarTask(e, task.taskEntity)}}/>
            ))}
        </Paper>
    </>);
}
